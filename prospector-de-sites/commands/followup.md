...
1. Acionar `fabi` (`agents/fabi.md`) por completo: verificar
...
2. Para cada elegível: `fabi` escreve o texto (máximo 4 linhas, tom
   gentil, sem preço, checklist anti-spam da skill `proposta-email`) e
   **passa pelo gate de LGPD (`agents/lia.md`, obrigatório e bloqueante
   — mesma regra do `/proposta`, RF-16)** antes de criar o rascunho/
   enviar via Gmail. Só se `lia` aprovar, o `carmem` persiste
   `contato_realizado -> follow_up` ou `follow_up -> follow_up`. Se
   bloquear, pule aquele lead nesta rodada e reporte o motivo.
3. Para quem já atingiu o limite sem resposta: `carmem` persiste
...
automatizar via Routine agendada (ver `agents/fabi.md`, seção
