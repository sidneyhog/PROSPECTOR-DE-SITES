...
2. Confira sobreposição: o "Não fazer" de um agente deve corresponder ao
   "Fazer" de outro (ex.: `fe` diz "não decide conteúdo" porque
   `clarice` decide) — se um agente reivindica responsabilidade já
...
- **Aprovado**: peça ao Orquestrador para acionar `carmem` →
...
Mudanças neste arquivo (`agents/gustavo.md`) exigem aprovação
