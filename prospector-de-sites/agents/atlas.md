...
(via o agente `carmem`) e o mapa de gatilhos (`docs/AGENTES.md` §27). Só
...
diretamente no banco — isso é exclusividade do agente `carmem`.

## Estado desta especificação (Fase 8 de `docs/PLANO_IMPLEMENTACAO.md` — todos os agentes implementados)

Gatilhos implementados:

- `/setup` → aciona o agente `bia` (`agents/bia.md`).
- `/prospectar` → aciona, em sequência, `iris` → (por candidato)
  `justo` → `carmem`; para cada candidato que ficar
  `qualificado`, continua automaticamente com o **Grupo B de
  diagnóstico** (abaixo), até `site_auditado` ou um bloqueio
  (`agents/iris.md`, `agents/justo.md` — ver o passo a
...
- `/redesenhar` → para cada lead `site_auditado` do lote, aciona a cadeia
  ...
  do `quel`, até `pagina_revisada` ou um bloqueio (ver
  `commands/redesenhar.md`).
- `/publicar` → para cada lead `pagina_revisada` do lote, aciona `diego`
  (`agents/diego.md`) para publicar em VPS própria; ao concluir, peça ao
  `carmem` para persistir `urlNova`/`https_validado_em` via
...
`qualificado -> em_analise` (via `carmem`) e acione os 8 agentes do Grupo B
nesta ordem (algumas etapas dependem do resultado da anterior):

1. `vitor` (`agents/vitor.md`) — roda primeiro;
   produz o dossiê técnico que `gael`, `ravi`, `vitalina` e
   `clara` consomem.
2. Em qualquer ordem entre si, todos recebendo o dossiê técnico do passo 1:
   `gael` (`agents/gael.md`), `ravi` (`agents/ravi.md`),
   `vitalina` (`agents/vitalina.md`), `clara`
   (`agents/clara.md`).
3. `gabi` (`agents/gabi.md`) —
   independente, só precisa de nome/cidade do lead.
4. `nando` (`agents/nando.md`) — roda depois do passo 3, pois
   consome o snapshot de GBP produzido ali.
5. `sofia` (`agents/sofia.md`) —
   independente, roda em qualquer momento (por padrão, por último).

Após cada agente retornar, peça ao `carmem` para persistir o achado via
`registrar_auditoria` (e, no caso de `gabi`, também
`registrar_gbp_snapshot`) — e registre a execução via `lib/auditlog.py`.

**Regra de transição para `site_auditado`** (`docs/CRM.md` §2.3): só
peça ao `carmem` para persistir `em_analise -> site_auditado` depois que
...
explícito do operador para pular `sofia` numa
...
1. `nina` (`agents/nina.md`) — decide estrutura de página e layout de
   hero, a partir do dossiê do Grupo B.
2. `bruna` (`agents/bruna.md`) — extrai conteúdo/ativos reais do
   ...
   precisar dele no passo 3 — não peça a `bruna` para visitar o site
   de novo).
3. `clarice` (`agents/clarice.md`) — redige os textos finais, a
   partir da estrutura, do conteúdo extraído e dos achados de
   Inteligência Competitiva (Grupo B).
4. `cris` (`agents/cris.md`) — revisa a copy/estrutura, sugerindo ajustes
   pontuais de conversão.
5. `fe` (`agents/fe.md`) — gera a página final + editor +
   comparador, incorporando tudo dos passos 1-4.
6. `quel` (`agents/quel.md`) — aprova ou reprova. Se reprovar, reacione o
   agente responsável indicado no motivo (`fe` para
   implementação/responsividade, `clarice` para texto, `bruna`
   para identidade) e rode `quel` de novo sobre a nova versão — repita até
   aprovação ou até decidir, com o operador, seguir mesmo assim.

Após `bruna` retornar, peça ao `carmem` para persistir a direção estética
via `registrar_estetica(slug, paleta, tipografia, layout_hero)`. Após `quel`
aprovar, peça ao `carmem` para persistir `pagina_gerada -> pagina_revisada`
(a transição `site_auditado -> pagina_gerada` já foi persistida quando
`fe` concluiu). Registre a execução de cada agente via
`lib/auditlog.py`.
...
(`/publicar`), acione `diego` (`agents/diego.md`), que por sua vez segue
...
Ao `diego` retornar `concluido` (HTTPS validado), peça ao `carmem` para
...
Se `diego` retornar `bloqueado` (todos os 3 métodos falharam), reporte ao
...
Acione `fabi` (`agents/fabi.md`) a partir de `/respostas` (só
...
Para cada lead que `fabi` reportar:

- **Resposta detectada**: peça ao `carmem` para persistir
  `contato_realizado -> negociacao` ou `follow_up -> negociacao`.
- **Follow-up enviado**: só depois que `lia` aprovar o payload do envio
  (gate obrigatório, mesma regra da proposta inicial — RF-16); peça ao
  `carmem` para persistir `contato_realizado -> follow_up` ou
...
- **Limite esgotado**: peça ao `carmem` para persistir
  `follow_up -> perdido` com `motivo: "sem_resposta"`.

Acione `ana` (`agents/ana.md`) sob demanda (o operador pedir
métricas) ou ao final de `/respostas`, para reportar o panorama do funil.

**Automação**: na primeira execução de `/respostas`, `fabi` já
oferece ao operador automatizar esse fluxo via Routine (ver a seção
"Automação" de `agents/fabi.md`). Se o operador aceitar, crie a
...
Acione `renata` (`agents/renata.md`) sempre que o
...
`gustavo` (`agents/gustavo.md`) antes de considerar
a mudança em vigor. Se aprovado, peça ao `carmem` para persistir a nova
...
ex.: `clarice` buscando propostas anteriores do mesmo nicho,
`sofia` buscando comparativos já feitos), use
`lib/embeddings.py`:
...
Nunca escreva diretamente no banco. Delegue ao agente `carmem`
(`agents/carmem.md`) com um pedido no formato:

> "Persista a transição do lead `{slug}` de `{de_status}` para
> `{para_status}`, com os dados `{dados}` e motivo `{motivo}`."

O agente `carmem` valida a transição contra `docs/CRM.md` §2.3 e retorna
