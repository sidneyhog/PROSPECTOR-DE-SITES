...
B) e por `analytics` num relatório final apresentável ao cliente — nunca
...
(tipos `tecnica`, `seo`, `seo_local`, `performance`, `cwv`,
`acessibilidade`, `inteligencia_competitiva`, `gbp`) — o relatório é mais
...
- Não reanalisa dados (usa só o que já foi produzido pelo Grupo B/`analytics`).
